...
function resetTimer() {
    ...
    // CR: no need to undefine the clearInterval.
...
// CR: pretty hard to read. better to use two different functions for cellClicked and cellMarked
    ...
    // CR: use (&&), ungood practice of elseif:

    if (currCell.isMarked && event.which === 1) return;
    // CR: supposed to be just >  if (currCell.isMarked ) return;
    // regular cell click
    ...
    if (!currCell.isMarked && event.which === 3) {
        ...
        var elMineCounter = document.querySelector('.mines-counter');
        elMineCounter.innerHTML = gCurrLevel.mines - gState.markedCount;
    } else if (currCell.isMarked && event.which === 3) { //CR : use &&
        // unmark cell 
        // update model
